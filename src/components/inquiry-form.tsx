...
const STORAGE_KEY = "bdf-inquiries";
...
export function InquiryForm({
  ...
  function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const prev = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? "[]") as Inquiry[];
      localStorage.setItem(STORAGE_KEY, JSON.stringify([inquiry, ...prev].slice(0, 20)));
    } catch {
      /* ignore quota */
    }
    setSent(inquiry);
    toast.success("Message prepared. Please send the email so we receive it.");
  }

  if (sent) {
    const subject = encodeURIComponent(
      `${SITE.shortName}: ${INTERESTS.find((i) => i.value === sent.interest)?.label}`,
    );
    ...
    return (
      <div className="rounded-2xl border border-gold/40 bg-cream p-6 shadow-soft">
        ...
        <p className="mt-2 text-muted">
          To make sure this reaches our team in Lugulu, please send it from your
          email. The message is ready — tap below to open it.
        </p>
        <div className="mt-5 flex flex-wrap gap-3">
          <Button asChild>
            <a href={mailto}>Send email to {SITE.email}</a>
          </Button>
    ...
    <form onSubmit={submit} className="flex flex-col gap-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <select
            ...
            className="flex h-11 w-full rounded-md border border-line bg-card px-3 text-base text-ink shadow-soft focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold"
          >
            {INTERESTS.map((item) => (
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <Textarea
          ...
          required
          placeholder="Tell us how you would like to walk with us."
        />
      </div>
      ...
      <p className="text-sm text-muted">
        This form prepares an email to {SITE.email}. Nothing is sent until you
        confirm it in your mail app.
      </p>
    </form>
  );
}
